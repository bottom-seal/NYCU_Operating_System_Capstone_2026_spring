...
static int request_page_for_chunk(int pool_idx) {
    ...
    base = (char *)page_addr_to_phys_addr(page);//base points to first byte of the requested page, cast to char so math in bytes (smallest unit in addr)
...
static void *dynamic_chunk_allocate(unsigned int size) {
    ...
    page = phys_addr_to_page_addr((void *)c);//find which page this chunk belongs to, page is a pointer for mem_map
...
static void remove_page_chunks_from_pool(struct page *page) {
    ...
    page_start = (unsigned long)page_addr_to_phys_addr(page);//start addr of this page in RAM
...
int find_mem_map_region(const void *fdt,
    ...
    kernel_start = (uintptr_t)_start;//from linker
    kernel_end = (uintptr_t)_end;
    regions[reserved_count].start = kernel_start;
    regions[reserved_count].end = kernel_end;
    reserved_count++;

    dtb_start = (uintptr_t)fdt;//from param
...
void page_alloc_init(const void *fdt) {
    ...
    mem_map = (struct page *)mem_map_start;
