...
static inline unsigned long irq_save(void) {
    ...
    asm volatile("csrr %0, sstatus" : "=r"(sstatus));//read out the old sstatus into C variable
    //csr clear: clear bit 
    asm volatile("csrc sstatus, %0" :: "r"(1UL << 1) : "memory"); // clear SIE in actual sstatus register
    return sstatus;//returns old sstatus
}

static inline void irq_restore(unsigned long flags) {//should input sstatus C variable
    asm volatile("csrw sstatus, %0" :: "r"(flags) : "memory");//write the stored flag (make sstatus.SIE enable, if it was enabled before irq_save)
}
