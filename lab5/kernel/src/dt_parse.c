...
uintptr_t get_plic_base(const void *fdt) {
    ...
    if (offset >= 0) {
        debug_uart_puts("DTB: found PLIC node at /soc/plic\n");
    } else {
        offset = fdt_path_offset(fdt, "/plic");
        if (offset >= 0)
            debug_uart_puts("DTB: found PLIC node at /plic\n");
    }

    if (offset < 0) {
        offset = fdt_path_offset(fdt, "/soc/interrupt-controller");
        if (offset >= 0)
            debug_uart_puts("DTB: found PLIC node at /soc/interrupt-controller\n");
    }

    if (offset < 0) {
        offset = fdt_path_offset(fdt, "/soc/interrupt-controller@e0000000");
        if (offset >= 0)
            debug_uart_puts("DTB: found PLIC node at /soc/interrupt-controller@e0000000\n");
    }

    if (offset < 0) {
        debug_uart_puts("DTB: PLIC node not found in known paths\n");
        return 0;
    }

    const void *prop = fdt_getprop(fdt, offset, "reg", &len);
    if (!prop || len < 8) {
        debug_uart_puts("DTB: PLIC node found but reg property invalid\n");
        return 0;
    }
